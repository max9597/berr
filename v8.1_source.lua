...
local bcs = loadstring(game:HttpGet("https://dogix.wtf/scripts/bconsole",true))()
local b64l = loadstring(game:HttpGet("https://dogix.wtf/scripts/libraries/b64.lua",true))()
local emojil = loadstring(game:HttpGet("https://dogix.wtf/scripts/emojilib",true))()
local lib = loadstring(game:HttpGet("https://zypher.wtf/UI-Lib",true))()
local valid = false
...
if is_sirhurt_closure then crash() end
function isdev(name)
    return name == "dogixism" or name == "AlexBoss_YTB" or name == "gun808" or name == "AncestorDevelopment"
end
function isdevdiscord(id)
    return id == 712942506412605473
    or id == 761779164607938562
    or id == 397937478167953408
    or id == 630914652032991244
    or id == 262986993862967296
end
...
-- Whitelist Handler
if forcewl and not dbgmode then
    if wlmethod == 0 then
        local x = pcall(function()game:HttpGet('http://localhost:26080',true)end)
        if x ~= false then
            local xstr = tostring(math.random(1,10000000))
            x = game:HttpGet('http://localhost:26080/?'..xstr,true)
            local remake = hash(b64(tostring(game:GetService'Players'.LocalPlayer.Name))..x)
            if _G["_"..x] == nil then
                crash()
            end
            if remake ~= game:HttpGet('http://localhost:26080/?a?'..xstr.."=".._G["_"..x],true) then
                crash()
            end
        else
            crash()
        end
    elseif wlmethod == 1 then
        if getgenv().BarkWhitelist == nil then crash() end
        local parts = getgenv().BarkWhitelist:split('|')
        local hsh = hash(b64l.encode(game.Players.LocalPlayer.Name).."BarkWin")
        local orignum = (tonumber(b64l.decode(parts[3]):split(" ")[1])/1000)
        local rnum = 0
        parts[2]:gsub(".",function(b)if tonumber(b) then rnum = rnum + tonumber(b)end end)
        local num = math.abs(tick()-math.floor(orignum/rnum))
        if num > 40000 or not hsh == parts[2] then
            wait(9e9)
            crash()
        end
    elseif wlmethod == 2 then
        ...
        local keyhash = game:HttpGet("https://dogix.wtf/scripts/lt2/barkdata/key_hash",true)
        if not keyhash:find(hashii(getgenv().BarkKey)) then
            Instance.new("Message",game.CoreGui).Text = "Script out of date! Get the new one from #releases in https://dogix.wtf/discord/"
            wait(1)
            crash()
        end
...
function _G.DogixLT2TP(x,y,z,key)
    ...
    if key ~= gkey and key ~= gkey + 1 then
        game.Players.LocalPlayer:Kick(" ! global key not detected! script denied access.\n")
...
local opts2 = wc:CreateSection("Welcome to "..cex.." "..cv.." | https://dogix.wtf/discord")
opts2:Create(
    "Toggle",
    "Infinite Range",
    function(state)
